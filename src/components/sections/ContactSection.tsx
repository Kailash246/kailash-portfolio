  ...
  const unifiedProfiles = [
    {
      ...
      handle: PERSONAL_INFO.socialLinks.email,
      url: `mailto:${PERSONAL_INFO.socialLinks.email}`,
      copyKey: 'personalEmail',
      icon: '✉️',
    ...
    },
    {
      type: 'email' as const,
      name: 'Academic Email',
      badge: 'ACADEMIC EMAIL',
      handle: PERSONAL_INFO.socialLinks.academicEmail,
      url: `mailto:${PERSONAL_INFO.socialLinks.academicEmail}`,
      copyKey: 'academicEmail',
      icon: '🎓',
  ...
  return (
    ...
    >
      ...
      <div className="space-y-8 sm:space-y-10">
        {/* Contact Form Card */}
        <div className="p-5 sm:p-8 lg:p-10 rounded-3xl bg-neutral-50 border border-neutral-200 space-y-4 sm:space-y-5">
          ...
          <h3 className="text-xl sm:text-3xl lg:text-4xl font-sans font-black text-[#111111] tracking-tight leading-tight">
            LET'S BUILD SOMETHING TOGETHER.
          </h3>

          <p className="text-xs sm:text-sm font-sans text-neutral-600 leading-relaxed max-w-2xl">
            Have an internship opportunity, a project to architect, or an engineering role to discuss? Send a direct message using the form below.
          </p>
        ...
        {/* Coding & Professional Profiles Directory */}
        <div className="space-y-3 sm:space-y-4">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-1 px-1">
            ...
            <span className="text-[10px] font-mono text-neutral-400">
              DIRECT INQUIRIES & VERIFIED PLATFORMS
            </span>
          </div>

          <div className="grid grid-cols-1 md:grid-cols-2 gap-3 sm:gap-4">
            {unifiedProfiles.map((item) => {
              if (item.type === 'email') {
                return (
                  <div
                    key={item.name}
                    className={`p-3.5 sm:p-4 rounded-2xl bg-white border border-neutral-200 transition-all group shadow-3xs hover:shadow-md flex items-center justify-between gap-3 ${item.color}`}
                  >
                    <a
                      href={item.url}
                    ...
                    <div className="flex items-center gap-1.5 shrink-0">
                      <button
                        ...
                        onClick={(e) => {
                          e.preventDefault();
                          e.stopPropagation();
                          handleCopy(item.handle, item.copyKey);
                        }}
                        className="w-8 h-8 rounded-xl bg-neutral-100 hover:bg-neutral-200 text-neutral-700 hover:text-black border border-neutral-200/80 flex items-center justify-center transition-all cursor-pointer"
                      ...
                      <a
                        href={item.url}
                        className="w-8 h-8 rounded-xl bg-neutral-900 hover:bg-black text-white flex items-center justify-center transition-all cursor-pointer shadow-3xs"
                        title="Send Mail"
                        aria-label={`Send mail to ${item.handle}`}
                      >
              ...
              return (
                <a
                  ...
                  target="_blank"
                  rel="noreferrer"
                  className={`p-3.5 sm:p-4 rounded-2xl bg-white border border-neutral-200 transition-all block group shadow-3xs hover:shadow-md ${item.color}`}
                >
          ...
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 text-[11px] sm:text-xs font-mono text-neutral-500 pt-5 sm:pt-6 border-t border-neutral-200/80">
